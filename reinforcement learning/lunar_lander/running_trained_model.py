...
#   loss = tf.reduce_mean(weighted_negative_likelihoods, name="loss_value")
   ...
   prob_0 = tf.shape(probabilities)[0]
   prob_1 = tf.shape(probabilities)[1]
   index_range = tf.range(0, tf.shape(probabilities)[0]) * tf.shape(probabilities)[1]
   indexes = tf.range(0, tf.shape(probabilities)[0]) * tf.shape(probabilities)[1] + action_holder
   responsible_probabilities = tf.gather(tf.reshape(probabilities, [-1]), indexes, name="probabilities_of_chosen_action")
...
#      env.render()
      ...
      # choose action stochastically allowing for some randomness
